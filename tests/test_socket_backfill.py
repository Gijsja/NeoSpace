...
def test_socket_send_message(app):
    ...
    assert (
        len(message_events) > 0
    ), f"No message events received. All events: {received}"
...
def test_socket_connect_rejected(app):
    """Test connection rejection when not authenticated."""
