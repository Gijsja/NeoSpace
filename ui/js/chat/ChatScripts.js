...
export async function loadAndRunScript(container, id) {
    ...
    try {
        ...
        if(data.ok) {
            ...
            // We allow cdnjs for libraries (p5, three) but block everything else.
            const csp = `<meta http-equiv="Content-Security-Policy" content="default-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; object-src 'none'; base-uri 'none';">`;

            let html = '';
            // Simplified runner templates (similar to playground)
            if(s.script_type === 'p5') {
                 html = `<!DOCTYPE html><html><head>${csp}<style>body{margin:0;overflow:hidden}</style><script src="https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.9.0/p5.min.js"><\/script></head><body><script>try{${s.content}}catch(e){document.body.innerHTML='<pre style="color:red">'+e+'</pre>'}<\/script></body></html>`;
            } else if(s.script_type === 'three') {
                 html = `<!DOCTYPE html><html><head>${csp}<style>body{margin:0;overflow:hidden}</style></head><body><script type="module">try{${s.content}}catch(e){document.body.innerHTML='<pre style="color:red">'+e+'</pre>'}<\/script></body></html>`;
            } else {
