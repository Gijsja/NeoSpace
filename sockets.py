...
def handle_join_room(data):
    ...
    emit("room_joined", {"room": room_name, "room_id": room_id})
...
def handle_send(data):
    ...
    if not check_rate_limit(
        user_id, action="message", limit=config_limit, window=config_window
    ):
    ...
    if row is None:
        print(f"Msg insert failed after {MAX_RETRIES} retries: {last_error}")
...
def backfill(data):
    ...
    if after == 0:
        ...
        # Optimization: Use subquery to fetch latest DESC, then re-sort ASC
        rows = db.execute(
        ...
        # Sync: Limit to 500 to prevent massive payloads
        rows = db.execute(
            """SELECT id, user, content, created_at, edited_at,
                      deleted_at, room_id
               FROM messages
