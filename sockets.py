...
def init_sockets(app):
    ...
    def handle_send(data):
        ...
        if row is None:
            print(f"Insert failed after {MAX_RETRIES} retries: {last_error}")
    ...
    def backfill(data):
        ...
        if after == 0:
            # Initial load: Get latest 100 messages (chronological)
            # We fetch DESC to get the newest, then reverse to send in order
            rows = db.execute(
                """SELECT id, user, content, created_at,
                          edited_at, deleted_at, room_id
                   FROM messages
            ...
            # Sync: Get messages since last known ID
            # Limit to 500 to prevent massive payloads if client far behind
            rows = db.execute(
                """SELECT id, user, content, created_at,
                          edited_at, deleted_at, room_id
                   FROM messages
